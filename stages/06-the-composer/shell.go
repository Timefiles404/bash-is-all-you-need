// Stage 06 — wiring the interactive shell.
//
...
// small enough to hold in your head, and a reader poking at compaction — or
// recording a trace to read back in the composer — wants a window that does not
// close when a value is missing, a key that stops a runaway turn, and a way to
// fix the endpoint without editing a file.
//
...
// the lesson never has to grow any of those in order to stay usable.
//
...
// return is a whole front end arriving four stages later without needing one
...
func (s *shellSession) banner() []string {
	...
	out := []string{
		"  stage 06 · the composer — stage 05, plus a way to look at what it did",
		"",
...
//
// The context is accepted and not used, because nothing on this stage's call
// path takes one: a ctx arrives in stage 10, where a stalled response is the
// subject. Until then the interrupt key ends the session between turns and
// cannot cut a request that is already in flight.
...
//
// Three things depend on the working directory and all three have to move
// together: where commands run, where memory is read and written, and what the
// system prompt says the directory is. Moving two of them is worse than moving
// none — an agent told it is in one directory while its commands run in another
// produces confident, wrong answers about a tree it cannot see.
func (s *shellSession) open(dir string) (string, error) {
	...
	s.wd = dir
	a.memoryDir = dir
	a.system = s.assemble(a.cfg.shell, dir)
...
// Everything in it that can move, moves with /open: the memory files are read
// out of the working directory, and the working directory is named in the
// prompt.
func (s *shellSession) assemble(shell, wd string) func() string {
	memory := ""
	if !s.opts.noMemory {
	...
	return func() string { return full }
}
...
// lastAssistantText is what -p prints: the final thing the model said.
