...
// 同的程序：各章要的界面得小到能整个装进脑子里，而拿压缩去戳、或者录一份
// trace 回头在编排台里读的人要的正好相反：一个不会因为少了个配置项就关掉的
// 窗口、一个按下去就能停住失控回合的键、一条不用编辑文件就能换端点的路。
//
// 而这正是这个目录里要先说清的一件事：它现在装着两个终端界面，只有一个是课
// 程讲的。阶段自己那个——tui.go、keys.go、term.go、views.go、width.go——就是
// docs/06-the-composer.md 一路走过的那一课：一个循环三十行的 trace 阅读器，
// 用 --composer 打开，小是故意的，这个文件一行都没动过它。tui/ 是那件工具：
// 同样三个函数，背后多了一块滚动输出区、一份设置文件和一次权限提问，没有章
// 节讲它——而正因为有它，那一课永远不必为了还能用就把这些东西长出来。
//
...
// 的，而回报是：四个阶段之后整个前端接了进来，那条总线一行都没改。
//
...
type shellSession struct {
	...
	// wd 是命令跑起来的地方。
	//
	// config 里没有工作目录这个字段：这个阶段没有任何地方设 cmd.Dir，所以决
	// 定一条命令在哪里跑的是进程自己的目录，而换掉它的是 os.Chdir。这个字段
	// 是横幅和 /status 报出来的那份拷贝，而写它的只有 open()。
	wd string
}
...
// open 把这场会话指到另一个目录上。
//
// 有三样东西依赖工作目录，而这三样必须一起挪：命令在哪里跑、记忆从哪里读
// 写、系统提示词说目录是哪个。挪两样比一样都不挪更糟——一个被告知自己在某个
// 目录、命令却在另一个目录里跑的 Agent，会对一棵它看不见的树给出自信而错误
// 的答案。
...
//
// 在外壳需要跑它第二遍之前，它在 main() 里就是一条直线。它里面会动的东西，
// 都跟着 /open 一起动：记忆文件是从工作目录里读的，而工作目录本身就写在提示
// 词里。
...
// knob 是一项能在运行时改的设置。
//
// 写成一张表，而不是一条命令一个，因为它们有八个，而且形状都一样：读一个
...
// lastAssistantText 就是 -p 打出来的东西：模型最后说的那句话。
