...
// 杀进程树，Unix 版。和阶段 01 一样没改——每个阶段都是独立快照，文件
// 就跟着一起走。
//
// 它解决的问题：`bash -c "npm start &"` 立刻就退了，但被扔到后台的 npm
// 还在跑，**而且**还占着 stdout 管道不放。只杀 shell，孙进程还活着，
// cmd.Wait() 也还堵在一根永远不会关的管道上。见 main.go 里的 runBash。
//
// Unix 从 v7 起就有答案：进程组。每个进程都属于某个进程组，子进程继承
// 它，而 kill(2) 只要收到负的 PID，就会向整组发信号。于是整份活儿靠一
// 个整数就能杀掉，我们从来不必去走进程树——这是好事，因为走进程树天生
// 带竞态（你枚举它的那一刻到你杀它的那一刻之间，进程可以 fork）。
package main
