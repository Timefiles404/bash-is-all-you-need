...
reader assemble one. Thirteen chapters, one per directory under the stage directories. Each
...
the stage directories must not be modified, so the port is an `-overlay`: `go build` takes a
...
`08-sandbox/code/bypass_test.go` — is **blocked at argv** and **allowed at
...
`02-see-everything/code/trace.go` writes and makes the same two decisions
...
|---|---|
| extract declarations from `*/code/**` | look a selection up in the build table |
...
every commit to the stage directories, the site is one refactor away from teaching something
