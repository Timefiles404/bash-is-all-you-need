...
func (r *renderer) OnEvent(e Event) {
	...
	case KindSandboxExec:
		// Only shown with --show-request, because a single pipeline can be six
		// of these and a `for` loop can be sixty. They are always in the trace,
		// which is where you want them: the question "what did that command
		// actually run" is asked after the fact, not while it scrolls past.
		if r.showRequest {
			r.p("  %s\n", r.c(cDim, "· exec "+oneLineDim(e.Command, 88)))
		}
