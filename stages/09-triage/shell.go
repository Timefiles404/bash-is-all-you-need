// Stage 09 — wiring the interactive shell.
//
...
// small enough to hold in your head, and a reader poking at triage and the
// fallback ladder wants a window that does not close when a value is missing,
// limits it can change without a restart, and a way to fix the endpoint without
// editing a file.
//
...
// return is a whole front end arriving seven stages later without needing one
...
func (s *shellSession) run() error {
	...
	// context.Background(), because this stage has no session context to pass
	// down: nothing below main() takes one until stage 10 adds the deadlines.
	// The shell still gives every turn a cancellable context of its own — see
	// submit for what Ctrl-C can and cannot reach from here.
	return s.app.Run(context.Background())
}
...
func (s *shellSession) banner() []string {
	...
	out := []string{
		"  stage 09 · triage — an error is a decision, not a string: retry, fall back, or stop",
		"",
...
//
// The context is the shell's, and nothing under this line reads one: runTurn
// grows a ctx parameter in stage 10, together with the deadlines that need it.
// So Ctrl-C mid-turn cancels a context no model call is listening to and the
// turn finishes anyway. Named _ so that is visible here rather than discovered.
...
// cannot see.
//
// Chdir is what actually moves the first of the four: no stage sets cmd.Dir, so
// a command inherits the process's directory and nothing else.
...
//
// A table rather than one command each, because there are eleven of them and
