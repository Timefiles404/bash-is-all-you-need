...
**Why does a timeout that kills the shell still hang the agent?**
Because `cmd.Wait()` does not wait for the process, it waits for the stdout pipe
to close, and the pipe stays open as long as any process holds the write end. A
backgrounded grandchild inherited that handle, so killing only the shell leaves
the pipe open and `Wait` blocks anyway. The rescue is hung by the thing it was
supposed to rescue you from, which is why the fix has to kill the whole process
group or job rather than one process.

**Why does giving up on the reap mean the captured output has to go unread?**
Because the five-second reap deadline exists for the case where `g.kill()` did
not work, and moving on leaks the `Wait` goroutine that owns the output buffers.
The copying goroutine may still be writing into them, so reading them anyway is
a data race whose symptom is garbled context weeks later. Leaking a goroutine is
survivable and wedging the agent is not, but racing on a buffer is worse than
either, because it fails quietly.

**Why is the Unix containment airtight where the Windows one is not?**
Because Unix sets the process group in the gap between `fork()` and `exec()`, so
the child is in its own group before its first instruction runs and nothing can
escape. Windows cannot assign a Job Object to a process that does not exist yet,
so `adopt()` has to run after `Start()`, leaving a window in which a grandchild
spawned by the shell's very first act would be outside the job. The repo takes
that window and documents it in the code rather than pretending it is closed.

**Why is a passing containment test not evidence that containment works?**
Because a test and the implementation can share the assumption that is wrong.
Git Bash keeps its own POSIX pid namespace, so `$!` prints an MSYS pid rather
than a Windows one, and `OpenProcess` on that number does not error — it queries
whatever unrelated process owns it, so a test built on `$!` passes while proving
nothing. That is why the suite also demonstrates the failure mode, and why the
kill was mutation-checked by replacing `TerminateJobObject` with a no-op.

**Why does truncation keep both ends of the output rather than the head?**
Because the part of a failing command that matters is usually the last few
lines. In the real run a 275KB log was truncated even after the model asked for
`tail -100`, and the single `FATAL` line survived only because the tail was
kept; head-only truncation would have delivered routine `INFO` lines and dropped
the one line the question was about. The notice also states how many bytes went
missing, because "1.4MB was removed" tells the model the command was the wrong
shape and "something was removed" does not.

**Why does a successful `json.Unmarshal` not mean the tool call is valid?**
Because Go fills absent keys with the zero value, so the observed payload
`{"raw_arguments":""}` unmarshals with no error into a struct whose `Command` is
`""`. "The model omitted the required field" and "the model sent an empty
string" become the same value, and the agent runs an empty command believing it
was asked to. Declaring the field as `*string` separates them: `nil` is absent,
`""` is empty, and both get rejected.

**Why is `stop_reason` not evidence about the body it arrived with?**
Because the envelope was generated by the same system that produced the body.
The recorded Anthropic response says `stop_reason: tool_use` for a generation
that was cut off, reports 136 output tokens against a requested `max_tokens` of
10, and carries a tool call that is not the schema the agent published. When the
envelope and the body disagree, the body is the one that will run on your
machine.

**Why is a denial returned as a tool result rather than an error?**
Because the turn should continue. In the recorded run the model proposed
`ls -la`, was denied, narrowed its proposal to `ls`, was denied again, and then
asked a sensible question — all inside one turn, because each refusal came back
as data. Returning an error from that path kills the session at the exact moment
a human was engaged enough to be watching.

**Why can the permission gate only ever show you a command string?**
Because a command string is all one bash tool has. A `write_file` tool could
render a diff, a `send_email` tool could show the recipient, an `edit` tool
could refuse a write to a file that changed since the model last read it, and a
read-only tool could be marked safe to run in parallel — while `bash -c` has the
same opaque shape whether it is `grep` or `git push`. One tool buys breadth
cheaply, and this is the bill: you pay it every time the harness wants to ask a
good question.

**Why are undecodable bytes replaced rather than passed through?**
Because they are not corrupt — they are correct in an encoding nobody told the
agent about, such as GBK from a native program on a Chinese Windows. Passing
them through hands the model mojibake it will reason about confidently, while
replacing them with U+FFFD makes the failure visible: the model sees `����` and
knows something went wrong. Real transcoding is a dependency this repo declines
to take, and fixing the encoding at the source is better than either.

---

## Questions to think about

These have no answer in the repo. They are the judgement calls the four fixes
above leave open.

1. The Windows job has a known escape window, and the answer here is to write it
   down and ship it. Where is your own line between a gap you document and a gap
   that has to be closed before anyone else runs the agent — and does that line
   move when the agent runs unattended?

2. The timeout is one number for every command, so a full test suite and an `ls`
   get the same allowance, and all the harness has to go on is a string. What
   would you key a per-command budget on, and what happens the first time you
   are wrong in the direction that kills a build at 90%?

3. Truncation bounds one command's output. Ten commands that each come in just
   under the budget do the same damage more slowly, and nothing in this chapter
   notices. Whose problem is that — the harness's, the system prompt's, or the
   model's — and what would you have to measure to decide?

4. A denial goes back as a sentence asking the model not to retry the command
   unchanged. That is advice, not a constraint. Where would you put a rule the
   model cannot talk its way around, and what does that cost you on the day the
   rule is wrong?

5. Every fix here assumes a model that adapts once it is told clearly what went
   wrong, and the runs in this chapter support that. Suppose one did not — the
   same 40MB dump, three turns running. What should change in the harness, and
   what would you have it do instead of handing back the same notice again?
