// 阶段 08——把交互式外壳接上来。
//
// 取名 tuishell.go，是因为这个阶段里的 shell.go 是另一个 shell：本章讲的那
// 个内嵌解释器。这个文件讲的是用户往里敲字的那个。
//
// 它不是课程的一部分，它把东西交出去的那个 tui/ 包也不是。没有任何一章讲它
// 们。它们存在，是因为"读一个阶段"和"在一个阶段里*干活*"要的是两个不同的程
// 序：各章要的界面得小到能整个装进脑子里，而拿沙箱或者技能去戳的人要的正好
// 相反：一个不会因为少了个配置项就关掉的窗口、一个按下去就能停住失控回合的
// 键、一条不用编辑文件就能换端点的路。
//
...
// 的，而回报是：六个阶段之后整个前端接了进来，那条总线一行都没改。
//
...
func (s *shellSession) run(ctx context.Context) error {
	s.app = tui.New(tui.Config{
		...
		// 这里不提供 Escape，而这是关于这个阶段的事实，不是关于界面的决定：
		// runTurn 要到阶段 10 才收 context，所以取消这件事根本没有东西可以够
		// 到。一行提示许诺一个按下去什么都不干的键，比一行从不提它的提示更糟。
...
func (s *shellSession) onExit(w io.Writer) {
	...
	// 沙箱那份清点跟在它后面，顺序和 main() 打这两样时一样。在外壳底下它只
	// 能放在这里：界面一返回，main() 也跟着返回，所以朴素行提示符那个循环末
	// 尾的那一行永远到不了。
	if a.sb != nil {
...
// submit 跑一个用户回合。
//
// context 收下了却没用，因为这行以下没有任何东西收它：runTurn 要到阶段 10
// 才长出 ctx 参数，跟需要它的那些期限一起。上面那个 Uninterruptible 声明的
// 就是这件事，而这就是全部的行为：回合进行中按 Escape 和 Ctrl-C，它们会把
// 这件事说出来，什么都不改，回合一直跑到最后。
...
func (s *shellSession) segments() []tui.Segment {
	...
	if a.sb != nil {
		// 一条命令落到哪个解释器手里，会改变它能做什么，所以它该待在 yolo 旁
		// 边，而不是只出现在 /status 里。
...
func (s *shellSession) status() []tui.Section {
	...
	// --sandbox 在启动时就定了，之后开不了也关不掉，所以它不是旋钮。它在这
	// 里，是因为它一开着，上面那行 `shell` 就不再是命令跑的地方了，而屏幕上
	// 没有别的东西说这件事。
	box := []Row{{Name: "sandbox", Value: sandboxMode(a.sb)}}
	if a.sb != nil {
		// 和 main() 退出路上打的是同一份清点，只是在会话还跑着的时候就能读
		// 到——而一条被拦下的命令，正是在那个时候才要紧。它自己那个
		// "sandbox: " 前缀被去掉了：这一行本来就有名字。
...
// open 把这场会话指到另一个目录上。
//
// 有五样东西依赖工作目录，而这五样必须一起挪：bash 在哪里跑、内嵌解释器在
// 哪里跑、记忆从哪里读写、技能在哪里建索引、系统提示词说目录是哪个。挪四样
// 比一样都不挪更糟——一个被告知自己在某个目录、命令却在另一个目录里跑的
// Agent，会对一棵它看不见的树给出自信而错误的答案。
//
// 沙箱的根是最容易忘掉的那一样，因为只有它不是进程自己的目录：一条沙箱里的
// 命令在哪里跑由 interp.Dir 决定，而 os.Chdir 对它毫无作用。
...
// knob 是一项能在运行时改的设置。
//
// 写成一张表，而不是一条命令一个，因为它们有十个，而且形状都一样：读一个
// 数，写一个数。**不**在这里的，是启动之后就改不动的那些——它找到的 shell、
// trace 的格式、沙箱开没开——而 /set 表态的方式是不把它们列出来，不是收下改
// 动然后不理。
type knob struct {
	name string
	help string
