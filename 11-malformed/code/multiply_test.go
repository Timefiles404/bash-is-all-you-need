...
// ---------------------------------------------------------------------------
// checkCall, on the task tool
// ---------------------------------------------------------------------------

// These cases are stage 07's TestParseTaskArgs, rerouted through stage 11's one
// boundary. The cases did not change, because the failures they describe did not
// change: a payload that unmarshalled cleanly and contains no task still spawns
// a subagent with an empty prompt, which is stage 01's bug with a network call
// attached.
//
// What changed is where the rule lives. There is no hand-written validator per
// tool any more, so the assertions below are assertions about taskToolDef's
// SCHEMA — the same map that goes into the request. A rule the model is not told
// about is no longer expressible here.
//
// The faults are asserted rather than just "an error", because the class is what
// decides who is told and how: faultCut means the model never finished writing
// the call, and telling it that its JSON was invalid would be a false diagnosis
// that costs a round trip.
func TestCheckCallOnTheTaskTool(t *testing.T) {
	def := taskToolDef()
	...
	cases := []struct {
		...
		raw        string
		wantFault  argFault
		mentions   string // substring of Detail, when a fault is expected
		wantDesc   string
		wantPrompt string
		wantDrop   []string
	}{
		...
		{
			name:      "no prompt key at all",
			raw:       `{"description":"survey the docs"}`,
			wantFault: faultSchema,
			mentions:  "prompt",
		},
		{
			name:      "empty object — what a truncated-but-parseable call looks like",
			raw:       `{}`,
			wantFault: faultSchema,
			mentions:  "prompt",
		},
		{
			// Schema-valid: present, and a string. The blank-prompt rule is not
			// a schema keyword, so it is not checked here — emptyPrompt is, and
			// TestEmptyPromptIsCaughtWhereTheSchemaCannotSeeIt covers it.
			name:       "prompt present but empty passes the schema",
		...
		},
		{
			...
			// spinner. Stage 11 removed `description` from `required` for this
			// reason; stage 07 declared it required and then defaulted it.
			name:       "description missing is not a fault",
			raw:        `{"prompt":"Find every TODO in the repo."}`,
			wantPrompt: "Find every TODO in the repo.",
		},
		{
			name:      "not JSON at all, and closed",
			raw:       `description: survey the docs`,
			wantFault: faultNotJSON,
		},
		{
		...
		},
		{
			...
			raw:       `{"raw_arguments":"{\"prompt\": \"List every"}`,
			wantFault: faultCut,
		},
		{
			name:      "raw_arguments holding the empty string",
			raw:       `{"raw_arguments":""}`,
			wantFault: faultCut,
		},
		{
			// §E13 measured models adding fields the schema forbids. Dropped,
			// not refused: the tool cannot read them, so removing them changes
			// nothing that runs, and a refusal costs a whole round trip.
			name:       "undeclared keys are dropped, not rejected",
			raw:        `{"description":"d","prompt":"go","model":"opus","budget":3}`,
			wantDesc:   "d",
			wantPrompt: "go",
			wantDrop:   []string{"budget", "model"},
		},
		{
			name:       "an empty argument string is a zero-argument call, not a parse error",
			raw:        ``,
			wantFault:  faultSchema,
			mentions:   "prompt",
			wantPrompt: "",
	...
	for _, c := range cases {
		...
		if c.wantFault != faultNone {
			if c.mentions != "" && !strings.Contains(got.Detail, c.mentions) {
				t.Errorf("%s: detail %q does not mention %q — this text is what the model reads, and if it "+
					"does not name the field the model's only option is to guess and retry",
					c.name, got.Detail, c.mentions)
			}
			if got.Args != nil {
				t.Errorf("%s: a rejected call still returned Args %v; a caller that ignores Fault dispatches on that",
					c.name, got.Args)
			}
			continue
		}
		if d := taskDescription(got); d != c.wantDesc && c.wantDesc != "" {
			t.Errorf("%s: description = %q, want %q", c.name, d, c.wantDesc)
		}
		if p := strArg(got, "prompt"); p != c.wantPrompt {
			t.Errorf("%s: prompt = %q, want %q — this string is the entire brief the subagent gets",
				c.name, p, c.wantPrompt)
		}
		if len(got.Dropped) != len(c.wantDrop) {
			t.Errorf("%s: dropped %v, want %v", c.name, got.Dropped, c.wantDrop)
			continue
		}
		for i := range c.wantDrop {
			if got.Dropped[i] != c.wantDrop[i] {
				t.Errorf("%s: dropped %v, want %v", c.name, got.Dropped, c.wantDrop)
				break
			}
		}
...
// The prompt is passed through untrimmed: the subagent's task is whatever the
// model wrote, whitespace included. Only *blankness* is a rejection, and that is
// the tool's rule rather than the schema's.
func TestEmptyPromptIsCaughtWhereTheSchemaCannotSeeIt(t *testing.T) {
	def := taskToolDef()
	...
	for _, raw := range []string{
		...
		got := checkCall(def, raw)
		if got.Fault != faultNone {
			t.Fatalf("%s: the schema rejected it as %q; this test exists to prove the schema CANNOT see blankness, "+
				"so a fault here means the check moved and the tool-level guard is now dead code", raw, got.Fault)
		}
		if !emptyPrompt(got) {
			t.Errorf("%s: emptyPrompt said no — a subagent is about to be spawned with no task, and it will burn a "+
				"whole context window working out that it has nothing to do", raw)
		}
	}

	kept := `{"description":"d","prompt":"  go read main.go  "}`
	got := checkCall(def, kept)
	if emptyPrompt(got) {
		t.Fatalf("%s: rejected as blank", kept)
	}
	if p := strArg(got, "prompt"); p != "  go read main.go  " {
		t.Errorf("prompt = %q; the surrounding whitespace is part of what the model wrote and is passed through", p)
	}
}

// The schema the model is shown and the rules the boundary enforces have to be
// the same statement, and stage 07's version of this test only checked ONE
// direction — "does the schema require everything the parser requires?" — which
// is why it passed while `description` was declared required and then defaulted.
//
// This checks both directions, by construction rather than by inspection: a call
// containing exactly the required fields must be accepted, and dropping any one
// of them must be rejected. A field that is declared required but not actually
// needed fails the first half; a field that is needed but not declared fails the
// second.
func TestTaskSchemaAndBoundaryAgreeInBothDirections(t *testing.T) {
	def := taskToolDef()
	...
	// Every required field present and nothing else: must be accepted.
	full := map[string]any{}
	for _, r := range req {
		full[r] = "x"
	}
	...
	if err != nil {
		t.Fatalf("marshalling the minimal call: %v", err)
	}
	if got := checkCall(def, string(raw)); got.Fault != faultNone {
		t.Errorf("a call containing exactly the required fields %v was rejected as %q (%s) — the model is being "+
			"told one thing and judged by another", req, got.Fault, got.Detail)
	}

	// Each required field removed in turn: must be rejected, and must name the
	// field it is missing.
	for _, drop := range req {
		...
		if err != nil {
			t.Fatalf("marshalling the call without %q: %v", drop, err)
		}
		got := checkCall(def, string(raw))
		if got.Fault == faultNone {
			t.Errorf("%q is declared required and a call without it was accepted — the declaration is decoration, "+
				"and a model that trusts it wastes tokens producing a field nothing needs", drop)
			continue
		}
		if !strings.Contains(got.Detail, drop) {
			t.Errorf("the rejection for a missing %q does not name it: %q", drop, got.Detail)
		}
	}
}
...
func TestDispatchNamesTheToolItDoesNotHave(t *testing.T) {
	...
	results, disp := a.dispatch(context.Background(), 1, []Block{
