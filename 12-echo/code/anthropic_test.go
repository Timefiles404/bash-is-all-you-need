...
// anthWireBlock is one content block as it appears on the wire. Stage 04 added
// CacheControl; everything else is stage 03's.
...
type anthWireBody struct {
	...
	System    []anthWireBlock `json:"system"`
	Stream    bool            `json:"stream"`
	Messages  []struct {
		Role    string          `json:"role"`
		Content []anthWireBlock `json:"content"`
	} `json:"messages"`
	Tools []struct {
		Name        string          `json:"name"`
...
func TestAnthropicBuildRequestEnvelope(t *testing.T) {
	p := anthProvider()
	req, body, err := p.BuildRequest(context.Background(), "you are a shell", []Msg{TextMsg(RoleUser, "hello")}, []Tool{anthBashTool()}, 700)
	if err != nil {
	...
	_, defaulted, err := p.BuildRequest(context.Background(), "", []Msg{TextMsg(RoleUser, "hi")}, nil, 0)
	if err != nil {
	...
	if d := anthDecodeBody(t, defaulted); len(d.System) != 0 || len(d.Tools) != 0 {
...
func TestAnthropicBuildRequestCollapsesToolResults(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, body, err := anthProvider().BuildRequest(context.Background(), "sys", tc.msgs, []Tool{anthBashTool()}, 700)
			if err != nil {
				t.Fatalf("BuildRequest: %v", err)
			}
			got := anthDecodeBody(t, body)
...
func TestAnthropicBuildRequestMessageShapes(t *testing.T) {
	t.Run("a following user turn merges into the tool_result message", func(t *testing.T) {
		...
		_, body, err := anthProvider().BuildRequest(context.Background(), "sys", msgs, nil, 700)
		if err != nil {
			t.Fatalf("BuildRequest: %v", err)
		}
		got := anthDecodeBody(t, body)
	...
	t.Run("a system message in msgs is a caller bug, reported loudly", func(t *testing.T) {
		...
		_, _, err := anthProvider().BuildRequest(context.Background(), "sys", []Msg{TextMsg(RoleSystem, "you are a shell"), TextMsg(RoleUser, "hi")}, nil, 700)
		if err == nil {
	...
	t.Run("no messages at all is refused before the network", func(t *testing.T) {
		...
		if _, _, err := anthProvider().BuildRequest(context.Background(), "sys", nil, nil, 700); err == nil {
			t.Fatal("want an error for an empty conversation")
		}
	})

	t.Run("a tool with no schema still gets an input_schema", func(t *testing.T) {
		_, body, err := anthProvider().BuildRequest(context.Background(), "", []Msg{TextMsg(RoleUser, "hi")}, []Tool{{Name: "now", Description: "the time"}}, 700)
		if err != nil {
			t.Fatalf("BuildRequest: %v", err)
		}
		got := anthDecodeBody(t, body)
...
func TestAnthropicBuildRequestArgumentBytes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, body, err := anthProvider().BuildRequest(context.Background(), "sys", msgs, []Tool{anthBashTool()}, 700)
			if err != nil {
				t.Fatalf("BuildRequest: %v", err)
			}
...
func TestAnthropicBaseURLTrailingSlash(t *testing.T) {
	...
	req, _, err := p.BuildRequest(context.Background(), "", []Msg{TextMsg(RoleUser, "hi")}, nil, 10)
	if err != nil {
		t.Fatalf("BuildRequest: %v", err)
	}
	if want := "https://opencode.ai/zen/go/v1/messages"; req.URL.String() != want {
