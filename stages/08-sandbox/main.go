// 阶段 08 —— 沙箱：Agent，带一个我们拥有的 shell，而不是我们只是启动
// 的那种。
//
// 这是可选的阶段，唯一一个有标准库之外的依赖的阶段。这一章在很大程度
// 上是关于为什么那个依赖赚了它的位置 —— 关于它**不**买什么，就是安全。
//
// 对阶段 07 的 diff 是两个文件和一个 flag。--sandbox 开启时，runCommand
// 会通过 sandbox.run 路由；下游一切都完全相同，因为 exec.go 从阶段 01
// 起，就一直把"运行一个命令"和"渲染它的结果"当作两个分开的工作。
package main
...
type agent struct {
	...
	// 阶段 08。nil 意思是"使用一个真实的 bash"；看 runCommand。
	sb *sandbox
