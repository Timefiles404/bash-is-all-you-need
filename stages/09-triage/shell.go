...
// 同的程序：各章要的界面得小到能整个装进脑子里，而拿分诊和降级梯子去戳的人
// 要的正好相反：一个不会因为少了个配置项就关掉的窗口、一批不用重启就能改的
// 上限、一条不用编辑文件就能换端点的路。
//
...
// 的，而回报是：七个阶段之后整个前端接了进来，那条总线一行都没改。
//
...
func (s *shellSession) run() error {
	...
	// 用 context.Background()，因为这个阶段没有会话 context 可往下传：在阶段
	// 10 加上期限之前，main() 底下没有任何东西收它。外壳仍然给每个回合配一个
	// 自己的、可取消的 context——从这里往下 Ctrl-C 够得着什么、够不着什么，见
	// submit。
...
// 的树给出自信而错误的答案。
//
// 真正把这四样里的头一样挪走的是 Chdir：没有哪个阶段设过 cmd.Dir，所以一条
// 命令继承的就是进程的目录，别的什么都没有。
...
// knob 是一项能在运行时改的设置。
//
// 写成一张表，而不是一条命令一个，因为它们有十一个，而且形状都一样：读一个
