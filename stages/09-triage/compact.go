...
func (c *compactor) run(p Provider, pol retryPolicy, httpc *http.Client, bus *Bus, msgs []Msg, cut int, baseChars int) ([]Msg, error) {
	...
	// 总结调用和其他每一次模型调用走的是同一套分类、同一个重试循环。阶段 09 就
	// 是它不再各自留一份更差的副本的地方——老的那一份根本不读响应体，所以一次失
	// 败的压缩永远只报得出 `http 500`，别的什么都没有。
	//
	// 刻意只给一级梯子：这个调用可以被重试，但不可以被降级。因为一次总结打了个
	// 嗝，就把整个会话搬到另一个供应商上，那不是恢复，那是惊吓——而且价格会在一
	// 个被告知"compacting"的用户脚下变掉。
	//
	// 也没有工具。见 flatten：摘要器不是 Agent，一定不能表现得像一个。
