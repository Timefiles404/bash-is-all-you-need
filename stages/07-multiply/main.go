// 阶段 07——Multiply：同一个循环，再叫一次。
//
// 两个特性，哪个都不是子系统：
//
//	子 Agent   一份新的 []Msg，另一套系统提示词，其余一切照旧——而且回
//	           来的只有**文本**。subagent.go。
//	技能       一个装 Markdown 文件的目录，加上系统提示词里的一段话，说
//	           它们存在。skills.go。
//
// 跟阶段 06 的 diff 很小。call() 问的是 a.tools()，而不是直接点名 bash；
// runTurn 里那四十行工具分发变成了对 dispatch() 的一次调用，它并发地跑子
// Agent，再按模型要求的顺序把结果交回来；Bus 长出了 Fork，好让一棵 Agent
// 树写进同一条有序流。
//
// **没有**变的才是重点。没有调度器，没有消息队列，没有 Agent 注册表，Agent
// 之间也没有协议。子 Agent 就是一次函数调用，返回值是一段话。
package main
