...
// paints the assertion output red and eats characters.

// TestWideRangesSorted guards inRanges' unchecked precondition. A binary search
// over an unsorted table does not crash; it silently misses, and the symptom is
// one character that is the wrong width in one font on one machine.
...
// TestBytesRunesColumnsAreThreeDifferentNumbers: these three counts disagree and
// only the last one lays out a screen.
...
// then we owe the caller the orphan column, because a cell one column short
// shears a table exactly as badly as one column long.
func TestTruncColsWideBoundary(t *testing.T) {
	...
	// produces something it considers aligned and the terminal does not.
	cases := []struct {
		s    string
		n    int
		want string
		...
		{"a你b", 2, "a "},
		// The reset comes before the pad, so the filler space is not painted with
		// an open background colour.
...
// forever — inside a redraw, so the symptom is a frozen UI with no stack trace.
//
