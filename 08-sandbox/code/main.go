// Stage 08 — Sandbox: the agent, with a shell we own instead of one we start.
//
// This is the optional stage and the only one with a dependency beyond the
// standard library. The chapter is largely about why that dependency earns its
// place — and about what it does NOT buy, which is security.
//
// The diff against stage 07 is two files and one flag. runCommand routes
// through sandbox.run when --sandbox is on; everything downstream is identical,
// because exec.go has kept "run a command" and "render its result" as separate
// jobs since stage 01.
package main
...
type agent struct {
	...
	// Stage 08. nil means "use a real bash"; see runCommand.
	sb *sandbox
...
func main() {
	var (
		...
		// The interactive shell, in tui/. Not part of the course; see tuishell.go.
	...
	// four of the five things that depend on the directory is worse than moving
	// none of them.
	...
	if *sandboxOn {
		...
		// Under the shell this line would land on the alternate screen beneath
		// the frame. /status carries the same fact instead, in the stage 08
		// section, where it can also be read later than the first second.
		if !shellMode {
