...
每一章的结尾都一样：先是能真跑一遍的练习，然后是这一章让你能回答的那些
问题——附简略答案，方便你自己对一遍——最后是几个这里没有答案的思考题。
...
| [12 Echo](docs/12-echo.md) | 最便宜的工具调用是没发出去的那次——以及在动手之前，拿你手边已经有的 trace 先审一遍它值多少 | ✅ built |
...
- 流式调用上三个时钟，而不是一个：`http.Client.Timeout` 管的是响应体读取，
  ...
  **直接打在面板上**，就在 TTFT 旁边，于是那个超时是量出来的余量，不是
  ...
  那一章还讲了量这个数最顺手的两种办法为什么量到的都是别的东西。
- 给工具调用的结果缓存，出厂就是**关着的**，因为做它的那一章同时也在量它：
  拿十六份录下来的会话回放，它会在 **107 条命令里命中 4 次**，省下 **401 毫秒**
  ——而同一批会话里模型花掉的是 864 秒。审计跑的是你手边已经有的
  trace，不需要 key，这正是要点：建缓存之前，你就能知道它对**你自己**的
  负载值多少。它什么时候真划算也量了——四个 Agent 同时读同一个文件，
  命中 21%——同时量到的还有那个一直在干真活的缓存：**83.9%**。
...
```sh
../agent -p "说说这个目录是什么"     # 一个回合，没有界面，跑完就退出
echo "同一件事" | ../agent           # 管道，和阶段 00 一样
../agent --no-tui                    # 各章讲的那个朴素提示符
```

这个外壳住在 `tui/` 里，而且**故意没有任何一章讲它**——见
[AGENTS.md](AGENTS.md) 里关于"唯一一个允许待在 `stages/` 之外的包"的说明。
...
- **不用 Agent 框架，也不用 TUI 框架。** 没有 LangChain、没有向量数据库、
  ...
  长度的区别，而这一章讲的正好就是这三件事。`tui/` 里那个交互式外壳守的
  是同一条规矩——它只有标准库和 `x/sys`；它之所以是一个包而不是某个阶段
  里的文件，恰恰因为它**不属于**这门课。
