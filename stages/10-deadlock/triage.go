...
type CallError struct {
	...
	// cause 有值，说明结束这次调用的是**本程序**自己的时钟——卡住看门狗、
	// 总期限，或者有人按了 Ctrl-C——而不是供应商做了什么。见 deadline.go。
	//
	// 它必须是单独的字段，因为这类失败到分类器手上时全都是
	// context.Canceled，而 Phase/Status 描述的是一次没能把"哪儿出了错"说
	// 完的调用。没有它，中断就是一次没归类的流失败：重试三次，然后降级到
	// 第二个供应商。按下停止，反而扇出去了。
	cause error
}
...
func (e *CallError) triage() Triage {
	// 先由我们自己的时钟回答。本程序自己结束的调用，不是关于供应商的证
	// 据，所以下面那些规则对它没有管辖权——而其中最要紧的那个是中断：不
	// 然给它归类的，就是那个将死的请求恰好带着的半成品状态码。
	...
	case phaseStream:
		// 线上断掉的流，和带来 error *事件* 的流，今天在 Go 里是同一个错
		// 误，但决策绝不能相同。
		//
		// Type == "" 是传输那一种：连接在响应体中途死了。值得再试一次。
		//
		// 有名字的 type 是供应商在响应中途故意发出来的，其中只有下面这四
		// 个的意思是"再问一次"。其余那些是因为我们发过去的东西才来的，再
		// 发一遍还会产生同一个事件——这是 5xx 陷阱在 phaseStream 上的孪
		// 生。
		if e.Type == "" {
			return TriageRetry
		}
...
func modelCall(ctx context.Context, p Provider, httpc *http.Client, bus *Bus, turn int,
	...
	// 总预算先套上去，所以它是这次调用自己那些时钟里最外面的一层，里面任
	// 何一层取消都还能报出自己的原因。两层包装都是**每次尝试**一份，不是
	// 每次调用一份：重试要是继承了第一次尝试剩下的时间，每一轮拿到的预算
	// 都更小，越需要成功就越快失败。
	if dl.total > 0 {
	...
	if err != nil {
		// 被取消的请求，从任何有意义的角度看都没上过线，但它返回的错误是
		// 包着 context.Canceled 的 *url.Error，对"是哪个时钟触发的"一个字
		// 都不说。原因说。
	...
	// 从这里往下，流是被盯着的。看门狗归 guardBody 所有，由 stopGuard 结
	// 束——每一条出去的路径上都要结束，顺利那条也算，因为 goroutine 活得
	// 比它的调用还久就是一处泄漏，泄漏量乘上子 Agent 的个数。
...
// cancelCause 报出这次调用的 context 为什么结束了，没结束就报 nil。
//
// 它是一行的包装，留成函数只是为了这个名字：四个调用点上写 `cause:
// cancelCause(ctx)`，一眼就知道在问什么；写成
// `cause: context.Cause(ctx)`，读者就会开始想，context 还开着的时候它
// 会怎样。
//
// 答案是什么都不会：context.Cause 在 context 被取消之前一直返回 nil。
// 早先的版本在这里加了一层
// `select { case <-ctx.Done(): ... default: return nil }` 的保护，还配
// 了段注释讲它为什么重要。变异测试把那段注释杀了——把整个 select 删掉，
// 一个测试都没变，因为那里根本没有行为可改。那层保护只是把标准库写在
// 文档里的约定，又对着它自己念了一遍。
	...
	for {
		...
		case TriageRetry:
			...
			if err := sleep(ctx, w); err != nil {
				// 退避中途被中断。就按它本来的身份——取消——报出
				// 来，不是按启动这次等待的供应商错误：会话在 4 秒
				// 的 sleep 里结束，不是因为 HTTP 503 才结束的，
				// trace 不该那么说。
