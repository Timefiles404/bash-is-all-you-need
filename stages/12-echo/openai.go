...
func (p *openaiProvider) assistantMessage(m Msg) oaiMessage {
	...
	for _, b := range m.Blocks {
		...
		//
		// 原样穿过去，只有**一个**例外，是阶段 11 加的：空字符串会变成
		// `{}`，因为 §E14 实测到这里的 `arguments: ""` 换来的是 HTTP
		// 400——而 400 是 fatal，所以历史里只要有一次零参数的工具调用，
		// 这个会话就到此为止。见 renderArgs。
...
// sseToolAccum 是一次工具调用在途中的状态。它不是对外返回的那个形状，因为它拿
// 着一样调用方绝不该看见的东西：start 事件是不是已经发出去了。
//
// `args` 用的是普通字符串，不是 strings.Builder，这更慢，而且是必须的：阶段 11
// 的 mergeArgs 有时得**替换**掉已经攒下的东西，而不是往后接，而 Builder 只能一
// 路长下去。
	...
	err := readSSE(r, func(f sseFrame) error {
		...
		for _, ch := range c.Choices {
			...
			for _, tc := range d.ToolCalls {
				...
				// 条毫无意义的零长度 delta，别让它进 trace。
				//
				// 碎片是合并进去的，不是原样追加：阶段 11 的 mergeArgs 就
				// 是三种线上方言被调和的地方；而 delta 事件带的仍然是原始
				// 碎片，好让 trace 记下的是到了什么，不是留下了什么。
