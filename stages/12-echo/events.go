...
const (
	...
	// Stage 12. What the result cache decided about one command.
	//
	// One kind with four verdicts rather than four kinds, because they are four
	// answers to one question — see cacheVerdict for why refused and miss are
	// nevertheless not the same answer.
	//
	// The load-bearing part is what this event replaces. A command served from
	// the cache emits THIS and does not emit command_start or command_end,
	// because no command started and none ended. It would have been half a line
	// of work to emit them anyway with a flag set, and it would have made every
	// trace in this repo a document that says a process ran when none did. A
	// trace is evidence or it is decoration.
