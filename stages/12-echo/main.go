...
func main() {
	...
	go func() {
		...
		// agent to stop, which means unwinding: killing commands, closing the
		// trace, printing the bill. If that unwind is itself stuck, the user
		// needs a way out that does not depend on the code that is stuck.
		signal.Stop(sigc)
	}()
