...
func cacheBuild(t *testing.T, on bool) anthWireBody {
	...
	_, body, err := p.BuildRequest(context.Background(), "you are a shell", cacheFixture(), []Tool{{Name: "bash", Schema: map[string]any{"type": "object"}}}, 512)
	if err != nil {
...
func TestEnablingCachingChangesNothingElse(t *testing.T) {
	...
	_, on, err := p.withCacheBreakpoints(true).BuildRequest(context.Background(), "sys", cacheFixture(), tools, 512)
	if err != nil {
		t.Fatal(err)
	}
	q := newAnthropicProvider("https://example.test", "k", "m")
	_, off, err := q.withCacheBreakpoints(false).BuildRequest(context.Background(), "sys", cacheFixture(), tools, 512)
	if err != nil {
		t.Fatal(err)
	}
...
func TestToolArgumentKeyOrderSurvives(t *testing.T) {
	...
	p := newAnthropicProvider("https://example.test", "k", "m")
	_, body, err := p.BuildRequest(context.Background(), "sys", msgs, nil, 512)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarkedBodyIsValidJSON(t *testing.T) {
	p := newAnthropicProvider("https://example.test", "k", "m")
	_, body, err := p.BuildRequest(context.Background(), "sys", cacheFixture(), nil, 512)
	if err != nil {
		t.Fatal(err)
	}
