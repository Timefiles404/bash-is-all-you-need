...
type renderer struct {
	...
	// invalid 是这次会话在边界上拒掉的工具调用数（阶段 11）。它跟会话总计放
	// 在一起，而不是放进单次调用的状态里，因为有意义的是比例：一次会话里十
	// 分之一的调用根本没走到 shell，那要么是 max_tokens 设得太低，要么是模
	// 型满足不了的 schema——而一次只看一个调用，这两样都看不见。
	invalid int

	// cacheHits 和 cacheBytes 是阶段 12 的结果缓存，跟上面几行 Usage 里的
	// CacheRead 不是一回事。那个是 provider 的 prompt 缓存，是账单上的折扣；
	// 这两个是这个进程没去跑的命令。两个功能都叫缓存，同一块面板，唯一的防
	// 线就是它们永远不出现在同一行上。
	cacheHits    int
	cacheMisses  int
	cacheStale   int
...
// idleNote 把阶段 10 的仪表格式化出来；没有可报的就什么都不出。
//
// 是什么都不出，不是 "idle max 0ms"：只有一帧的流没有间隔可报，而 0 读
// 起来像"它从没停过"，不是"没有东西可测"。和价格未知时那个破折号是同一
// 条规矩。
...
func (r *renderer) OnEvent(e Event) {
	...
	case KindToolCallInvalid:
		// 既打印也计数，而有用的是计数这一半。一次被拒的调用是噪声；一次会话
		// 里十分之一的调用都是 `cut`，那就是 max_tokens 太低了——这块面板上再
		// 没有别的东西会告诉你这件事。
		r.invalid++
	...
	case KindResultCache:
		// 只有命中会打印，也只有命中在这里计入面板。另外三种判定是会话里的
		// 常态——冷缓存全是未命中，真在干活的会话大半是拒绝——面板要是把它们
		// 一条条念出来，命令输出就被自己的账本埋掉了。它们全在 trace 里，你
		// 想知道命中率为什么是这个数，就去那里看。
		//
		// 命中要打印，是因为它是这块面板上唯一一行在描述**没有发生**的事；
		// 看不到它的读者，会奇怪这条命令怎么既没输出也没耗时。
...
func (r *renderer) SessionSummary(finalPrompt int) {
	r.p("\n%s\n", r.c(cDim, "  ── session ──────────────────────"))
	// malformed 的计数只在非零时才打印。每份总结上都常驻一行 "0 malformed"，
	// 只会训练人不再读这一行——而一个真正要紧的数字，要的恰恰是反面。
	if r.invalid > 0 {
	...
	// 结果缓存哪怕什么都没干也要报，这跟上面两块遵守的规则正好相反，而这个例
	// 外就是阶段 12 的要点。"0 retries" 值得藏起来，因为一次没有重试的会话就
	// 是一次什么都没出错的会话。"0 hits" 值得打出来，因为一个从不命中的缓存
	// 看起来**跟一个正常工作的缓存一模一样**——没有错误答案，没有报错，任何日
	// 志里都没有一行——把这两者分开的只有这一行。只有真的开了缓存才会出现。
