...
// cspArgs 是故意**压紧**的 JSON。
//
// OpenAI 适配器把 Block.Args 当 JSON 字符串逐字节透传；Anthropic 适配器
// 把同一串字节当对象拼进去，途中 encoding/json 会压掉无意义空白。于是冒
// 号后面那个空格，恰好是两种线上格式真正有理由不一致的地方，而这个文件
// 要管的是除此之外的一切。`2>&1` 放在里面是故意的：两个适配器都用
// SetEscapeHTML(false) 编码，所以 shell 重定向必须以本来的样子活下来，
// 不能变成 \u003e\u0026。
...
// cspCanonArgs 把每个 tool_call 的 Args 重写成一种规范编码。
//
// 它的存在是因为这个文件**外面**有个缺陷，而这个缺陷值得在读者会绊到的
// 地方点名。TraceWriter 记录事件用的是普通 json.Marshal，它会把 `<`、
// `>`、`&` 做 HTML 转义——包括装着请求体的那段 json.RawMessage 里面，而
// 那段正是两个适配器故意用 SetEscapeHTML(false) 编出来的。字符串没事
// （解码能回来），但 Anthropic 视角读工具调用的参数时，读的是 `input`
// 的**原始**字节，所以记下来的 `2>&1` 回来变成 `2\u003e\u00261`。于是
// trace 和真正 POST 出去的字节不再逐字节相同，而线上视角——它全部的承诺
// 就是"逐字节"——显示的是转义后的样子。
//
// 在这里做规范化，是为了让这个测试讲的还是它要测的那趟往返，而不是那个
// bug；它没有把 bug 藏起来，等 trace.go 不再转义之后，它照样一个字不改
// 就能过。
