...
const (
	...
	// 失败与恢复（阶段 09）。三种 kind，理由和压缩有三种一样：它们回答的是不同
	// 的问题，而其他每个实现都塌缩成一行日志的那一个，正是你需要的那一个。
	//
	//	call_error  什么坏了，以及我们对它做了什么决策
	//	retry       我们正在等、要等多久、这个数字是谁的
	//	provider    从这里开始由谁服务调用，价格如何
	//
	// KindCallError 不是 KindError。KindError 是终局——会话在告诉人类它失败了。
	// 一个 call_error 是一次尝试失败了，还附着一个决策，而它们大多数后面跟着的
	// 是成功。把它们当错误发出去，等于训练人们忽略"错误"这个词。
...
// ProviderInfo 就是：谁服务了这次调用，以及他们的 token 什么价钱。
//
// 它搭在 KindProvider 上，因为一次降级会在会话中途改变计价基准。没有它，面板会
// 一直用第一个供应商的费率给第二个供应商的 token 计价——一份自信地算错的成本报
// 告，比一份承认自己不知道的更糟——而一条被重放的 trace 根本无从查起。顺带一
// 说，这也是 trace 第一次记录下它究竟由哪个端点产出。
...
type Event struct {
	...
	// 失败记账（阶段 09）。请求根本没拿到响应时 Status 是 0，这和"响应是 0"是两
	// 个不同的事实，也是它没有被折进 Text 的原因。
	//
	// ErrType 是供应商的 `error.type` 原文，刻意不做规范化：状态码分不出一个写
	// 错的模型名和一把被吊销的密钥时（§D11），靠的就是这个字段，所以一条把它规
	// 范化掉的 trace，等于扔掉了自己那个决策的证据。Phase 是这次调用在哪个环节
	// 坏掉的：build | connect | status | stream。
	//
	// 它不是 Status 旁边的装饰，两者的差别是钱。在 `status` 或 `connect` 上的失
	// 败，是在模型生成任何东西之前就被拒了，所以这次尝试是免费的。在 `stream` 上
	// 的失败拿到了它的 200 和它的 token，而那些 token 无论字节有没有到都要
	// 计费——所以这就是那个决定一次重试有没有花钱的字段，没有它，面板就会算错。
	// Status 替代不了：一次连接失败和一次流断裂都带着 Status 0。
	Status  int    `json:"status,omitempty"`
	Phase   string `json:"phase,omitempty"`
	ErrType string `json:"err_type,omitempty"`
	Triage  string `json:"triage,omitempty"`  // retry | fallback | fatal
	Attempt int    `json:"attempt,omitempty"` // 从 1 开始计数，跨整个梯子累计

	// Provider 说的是从这里开始由谁来服务调用。启动时设置，每次降级时也设置；
	// 价格为什么跟着它走，见 ProviderInfo。
