...
// 同的程序：各章要的界面得小到能整个装进脑子里，而拿分诊或者结果缓存去戳的
// 人要的正好相反：一个不会因为少了个配置项就关掉的窗口、一个按下去就能停住
// 失控回合的键、一条不用编辑文件就能换端点的路。
//
...
// 的，而回报是：十个阶段之后整个前端接了进来，那条总线一行都没改。
//
...
func (s *shellSession) open(dir string) (string, error) {
	...
	// 结果缓存这里什么都不用做，而这句值得写下来，因为它看着像是该做点什么。
	// 工作目录在 key 里，所以在这里攒下的每一条，从新目录根本就够不着——这是
	// 对的，因为 `ls` 的答案是关于一个目录的事实，不是关于一条命令的。
	//
...
func (s *shellSession) rebuildProvider() (string, error) {
	...
	// 抓下来的那份环境在这里也刷新，不只在 open() 里刷。
	//
	// /provider-apikey 会调 os.Setenv，而环境之所以进结果缓存的 key，正是因为
	// 它是一条命令继承的东西的一部分。让这份快照留着旧的，等于拿一份外壳已经
	// 不用的环境去给新条目算 key——谎不大，可撒在阶段 12 花了一整章论证"key
	// 必须完整"的那个地方。
...
// knob 是一项能在运行时改的设置。
//
// 写成一张表，而不是一条命令一个，因为它们有十七个，而且形状都一样：读一个
// 数，写一个数。**不**在这里的，是启动之后就改不动的那些——它找到的 shell、
// trace 的格式、缓存的大小——而 /set 表态的方式是不把它们列出来，不是收下改
// 动然后不理。
type knob struct {
	name string
	help string
...
func (s *shellSession) knobs() []knob {
	...
	return []knob{
		...
		{"cache", "serve a repeated read-only command from the result cache", func() string { return onOrOff(a.echo != nil) }, func(v string) error {
			...
			case !b:
				// 是丢掉，不是清空。一个关掉又开回来的缓存，绝不能把它在没人
				// 盯着见证还成不成立的那段时间里攒下的答案递出去。
				a.echo = nil
			}
			return nil
		}},
