...
// an internal detail.
func (c *compactor) run(p Provider, pol retryPolicy, httpc *http.Client, bus *Bus, msgs []Msg, cut int, baseChars int) ([]Msg, error) {
	...
	// The summarising call goes through the same classification and the same
	// retry loop as every other model call. Stage 09 is where it stopped having
	// its own, worse copy of both — the old one read no response body, so a
	// failing compaction could only ever report `http 500` and nothing else.
	//
	// A one-rung ladder, deliberately: this call may be retried and may not be
	// failed over. Moving the whole session onto another provider as a side
	// effect of a summarisation hiccup is not a recovery, it is a surprise —
	// and the prices would change under a user who was told "compacting".
	//
	// No tools, either. See flatten: the summariser is not an agent and must
	// not be able to act like one.
	res, err := retryLoop(bus, 0, pol.forCompaction(), newLadder(rung{p: p}), time.Sleep, nil,
		func(pr Provider) (*CallResult, error) {
			return modelCall(pr, httpc, bus, 0, summarySystem,
