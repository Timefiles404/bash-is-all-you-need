// Stage 12 — wiring the interactive shell.
//
...
// small enough to hold in your head, and a reader poking at triage or the
// result cache wants a window that does not close when a value is missing, a
// key that stops a runaway turn, and a way to fix the endpoint without editing
// a file.
//
...
// return is a whole front end arriving ten stages later without needing one
...
func classOf(e Event) tui.Class {
	...
	case KindToolResult, KindResponseEnd, KindUsage, KindRetry,
		KindResultCache, KindMemoryLoaded, KindSkillsIndexed,
...
func (s *shellSession) banner() []string {
	s.mu.Lock()
	a, pname, pcfg := s.a, s.pname, s.pcfg
	s.mu.Unlock()

	out := []string{
		"  stage 12 · echo — a result cache, and the measurement that says not to switch it on",
		"",
		fmt.Sprintf("  cwd       %s", a.cfg.wd),
...
func (s *shellSession) segments() []tui.Segment {
	...
	out = append(out, tui.Segment{Label: "in", Value: shortDir(a.cfg.wd), Tone: tui.ToneMuted})
	return out
}
...
func (s *shellSession) status() []tui.Section {
	...
	cache := []Row{
		{Name: "result cache", Value: onOrOff(a.echo != nil)},
	}
	if a.echo != nil {
		cache = append(cache,
			Row{Name: "hits", Value: strconv.Itoa(v.cacheHits), Note: fmt.Sprintf("%d ms of command time not spent", v.cacheSaved)},
			Row{Name: "misses", Value: strconv.Itoa(v.cacheMisses)},
			Row{Name: "stale", Value: strconv.Itoa(v.cacheStale)},
			Row{Name: "refused", Value: strconv.Itoa(v.cacheRefused), Note: "not eligible, and never will be"})
	}
...
func (s *shellSession) open(dir string) (string, error) {
	...
	a := s.a
	a.cfg.wd = dir
	a.cfg.env = os.Environ()
	a.memoryDir = dir
	sys, stable := s.assemble(a.cfg.shell, dir)
	a.system, a.stable = sys, stable

	// The result cache needs nothing done to it, and that is worth saying
	// because it looks like it should. The working directory is in the key, so
	// every entry collected here is simply unreachable from the new directory —
	// which is correct, since the answer to `ls` is a fact about a directory and
	// not about a command.
	//
...
func (s *shellSession) rebuildProvider() (string, error) {
	...
	// The captured environment is refreshed here, not only in open().
	//
	// /provider-apikey calls os.Setenv, and the environment is part of the
	// result cache's key precisely because it is part of what a command
	// inherits. Leaving the snapshot stale would key new entries on an
	// environment the shell no longer has — a small lie, but one in the exact
	// place stage 12 spent a chapter arguing that the key has to be complete.
	s.a.cfg.env = os.Environ()
		...
		},
		{
			...
			Run: func(_ context.Context, arg string, w io.Writer) error {
				...
				default:
					...
					// The provider event is re-emitted so the new file opens
					// with the fact every reader needs first: who is answering,
					// and at what price. Without it a trace started mid-session
					// has costs in it that cannot be reconstructed.
					s.mu.Lock()
					lad := s.a.lad
					s.mu.Unlock()
					if lad != nil {
...
//
// A table rather than one command each, because there are seventeen of them and
...
func (s *shellSession) knobs() []knob {
	...
	return []knob{
		...
		{"cache", "serve a repeated read-only command from the result cache", func() string { return onOrOff(a.echo != nil) }, func(v string) error {
			b, err := parseOnOff(v, a.echo != nil)
			if err != nil {
				return err
			}
			switch {
			case b && a.echo == nil:
				a.echo = newResultCache(s.opts.cacheEntries, s.opts.cacheBytes, s.opts.cacheTTL)
			case !b:
				// Dropped rather than emptied. A cache that is switched off and
				// then on again must not serve an answer it collected while
				// nobody was watching whether its witnesses still held.
				a.echo = nil
			}
			return nil
		}},
