...
**抽象好不好，为什么要看"主循环动了没有"，而不是看它编不编得过？**
编得过的设计照样能把供应商的词汇裹在里面，而这条漏子要等第二套协议到场
才看得见——那正好是修起来最贵的时刻。阶段 03 的循环就是阶段 02 的循环
换掉一套词汇，此外什么都没动。循环里第一条按协议名分岔的 `if`，就是那
一百条的开头。

**中立语言里为什么没有 `RoleTool`？**
因为两套协议装工具结果的形状不兼容：一套每个调用发一条单独的消息，另一
套把所有结果收进同一条 `user` 消息。没有哪个中立角色能同时表示这两种，
所以中立形式干脆两个都不要——工具结果是 Block，拿什么消息形状装它，各自
的适配器自己决定。`Msg` 里装的是块而不是扁平的字符串，理由也在这儿。

**`Block.Args` 为什么装原始 JSON 字符串，而不是解码好的 map？**
因为一套协议要 JSON 字符串，另一套要 JSON 对象，原始字节是唯一能同时喂
给两边、又不必重新序列化的形式。Go 的 map 遍历顺序不稳定，同一个值走一
趟"解码再编码"，出来的字节就可能变了。字节一变，prompt 前缀跟着挪位，
而下一整章讲的那个缓存就此作废。

**`Stop` 已经规范化过了，为什么还要留着 `RawStop`？**
因为信封会说谎：在这个网关上，工具调用在 `max_tokens` 处被截断，回来的
是 `stop_reason: "tool_use"` 加一段根本没法用的 body。`Stop` 记的是
Agent 当时相信了什么，`RawStop` 记的是它实际被告知了什么，两者之间那道
缝就是 bug。把手上唯一的证据规范化掉，剩下的东西不够你诊断。

**认不出的停止原因，为什么映射到 `StopUnknown` 而不是 `StopEndTurn`？**
因为一台状态机只要把认不出的东西都归进"大概没事"，早晚有一天，它会把一
次拒答、一次配额事件、或者一种新出的安全停止也归进"大概没事"。
`StopUnknown` 的代价是你要多写一个分支去处理；另一条路的代价是一个错的
答案，而且没有任何东西会报告它。

**`Usage` 为什么一个字都不用改就活过了第二套协议？**
因为它的字段名描述的是数字的含义，而不是某一家 API 管它叫什么——`Input`
的意思是"按全价计费"，不是 `prompt_tokens`。它是在阶段 02 定下来的，比
第二套协议的出现早整整一章，而这不是有远见。描述含义的名字能活过第二种
实现；从供应商 JSON 里照抄的名字活不过。

**把 `prompt_tokens` 直接抄进 `Input`，为什么是测试抓不住的 bug？**
因为一套协议的 `prompt_tokens` 是总数，`cached_tokens` 嵌在它里面；另一
套的 `input_tokens` 本来就已经是扣掉缓存后的余量。误差的大小恰好等于缓
存命中量，所以冷请求上它是零，测试里看着完美。它把 506 token 的 prompt
报成了 698，而你的缓存做得越好，它就错得越离谱。

**配置文件里为什么写 `api_key_env`，不写 `api_key`？**
因为配置文件早晚会被提交上去，没有一个逃得掉，而唯一靠得住的防线，是让
密钥在这个格式里根本没地方可坐。没有配置文件的时候，环境变量那条路照样
能走，所以最简单的情况永远不必先弄出一份文件来。

**为什么 SSE 分帧两个适配器共用，负载解析却不共用？**
因为切口正好落在机制结束、主张开始的地方。一套协议只发 `data:` 行，末尾
一个 `[DONE]` 哨兵，哨兵之后还有一帧；另一套发 `event:` 加 `data:`，没
有哨兵，消息两头各来一个 `ping`。把行从线上读出来，两边是同一件活；判断
这些行是什么意思，不是。

**两种写法模型读到的字符串一样，为什么还要关掉 Go 的 HTML 转义？**
因为服务端会解码，所以变的不是模型的输入，而是人看到的东西——跑 shell
的 Agent，命令里满是 `2>&1`、`>/tmp/out` 和 `<<EOF`，转义之后在请求检查
器里没法看。另外，供应商的缓存 key 散列的是原始字节还是解码后的内容，我
们并不知道，而不知道恰恰是要求一致的理由，不是拿来猜的理由。这两个适配
器一开始就不一致，那四行代码去掉的正是这处瑕疵。
...
1. 这里两套协议都流式，都带文本和工具调用，回合结构也一样，所以中立语言
   里一个 `Msg` 就罩得住。假设来了第三套，带着前两套都没有的概念——比如
   在服务端执行的工具，或者分成好几股流回来的响应。在动手写之前，你怎么
   判断这个概念该进中立语言，还是留在适配器里不外传？

2. "别把手上唯一的证据规范化掉"，字面照做就是把每样东西的原始形式都留一
   份，也就是每个响应都存两遍。你在哪儿收手？什么样的字段值得存两份，你
   又要怎么发现自己判断错了？

3. "主循环里不许出现供应商的词"这条规则是有价钱的。找一个只有一家有、另
   一家没有的能力，决定你要不要把它暴露出来——前提是答案不能是主循环里
   的一条 `if`。

4. 差异表里每一行都是盯着线上抓出来的，不是从规范上念的——它成立是因为
   有人真的把请求跑过一遍。哪天某家供应商悄悄改掉其中一条行为，会发生
   什么？你要怎么赶在用户之前发现？

5. 适配器吸收的是形状上的差异。差异到什么程度就不再是形状问题了——不支持
   流式的供应商，不支持工具调用的，或者上下文窗口小一个数量级的——到哪
   一步你要的就不是第三个适配器，而是另写一个 Agent？
