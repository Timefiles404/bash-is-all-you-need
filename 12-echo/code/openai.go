...
func (p *openaiProvider) BuildRequest(ctx context.Context, system string, msgs []Msg, tools []Tool, maxTokens int) (*http.Request, []byte, error) {
...
func (p *openaiProvider) assistantMessage(m Msg) oaiMessage {
	...
	for _, b := range m.Blocks {
		...
		//
		// Passing straight through with ONE exception, added in stage 11: the
		// empty string becomes `{}`, because §E14 measured `arguments: ""` as an
		// HTTP 400 here — and a 400 is fatal, so a single zero-argument tool
		// call in the history ends the session. See renderArgs.
		call.Function.Arguments = renderArgs(b.Args)
...
// shape because it holds one thing the caller must never see: whether the start
// event has already gone out.
//
// `args` is a plain string rather than a strings.Builder, which is slower and
// necessary: stage 11's mergeArgs sometimes has to REPLACE what accumulated
// instead of extending it, and a Builder can only ever grow.
	...
	err := readSSE(r, func(f sseFrame) error {
		...
		for _, ch := range c.Choices {
			...
			for _, tc := range d.ToolCalls {
				...
				// a meaningless zero-length delta out of the trace.
				//
				// Fragments are merged rather than appended: stage 11's
				// mergeArgs is where the three wire dialects are reconciled,
				// and the delta event still carries the raw fragment so the
				// trace records what arrived, not what was kept.
				if tc.Function.Arguments != "" {
					acc.args = mergeArgs(acc.args, tc.Function.Arguments)
					emit(Event{
