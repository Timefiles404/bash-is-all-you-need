...
// ---------------------------------------------------------------------------
// checkCall，作用在 task 工具上
// ---------------------------------------------------------------------------

// 这些用例就是阶段 07 的 TestParseTaskArgs，改道走阶段 11 那唯一一道边界。用例
// 本身没变，因为它们描述的故障没变：一份能干净反序列化、里面却没有任务的载荷，
// 照样会拿一个空 prompt 起一个子 Agent，那就是阶段 01 的那个 bug，外加一次网络
// 调用。
//
// 变的是规则住在哪儿。每个工具一份手写校验器的做法没有了，所以下面这些断言，断
// 的是 taskToolDef 的 **schema**——就是进到请求里去的那张 map。没告诉过模型的规
// 则，在这里已经写不出来了。
//
// 断言断到具体的故障分类，而不是只断"有个错误"，因为分类决定的是告诉谁、怎么告
// 诉：faultCut 的意思是模型压根没把这次调用写完，这时候跟它说它的 JSON 非法，
// 就是一次误诊，代价是一个往返。
func TestCheckCallOnTheTaskTool(t *testing.T) {
	...
	cases := []struct {
		...
		raw        string
		wantFault  argFault
		mentions   string // 期望有故障时，Detail 里该含的子串
		wantDesc   string
		wantPrompt string
		wantDrop   []string
	}{
		...
		{
			// schema 上合法：字段在，是字符串。"prompt 空白"这条规则不是 schema
			// 关键字，所以这里不查它——查它的是 emptyPrompt，
			// TestEmptyPromptIsCaughtWhereTheSchemaCannotSeeIt 覆盖了这一段。
		...
		{
			// 标签缺了只是门面问题——它就是用户在转圈图标旁边看到的那行字。阶
			// 段 11 把 `description` 从 `required` 里拿掉，就是为了这个；阶段 07
			// 声明它必填，转头又给它兜了默认值。
		...
		{
			// docs/wire-notes.md §A3c：工具调用被截短时，网关真的会发这个。它是
			// 合法 JSON，里面没有任务，而"有东西被截断了"的**唯一**证据就是这个
			// 键——stop_reason 照样写着 "tool_use"。
		...
		{
			// §E13 实测到模型真的会加 schema 明令禁止的字段。丢掉，而不是拒绝：
			// 工具根本读不到它们，删掉不会改变任何实际跑的东西，而拒一次要赔上
			// 一整个往返。
...
// prompt 原样传过去，不做 trim：子 Agent 的任务就是模型写的那些字，空白也算。
// 只有*全空白*才会被拒，而那是工具自己的规则，不是 schema 的。
...
// 给模型看的 schema，和边界上真正执行的规则，必须是同一句话。阶段 07 那一版的
// 这个测试只查了**一个**方向——"解析器要的，schema 是不是都要求了？"——所以
// `description` 一边被声明成必填、一边又被兜了默认值，它照样是通过的。
//
// 这里两个方向都查，而且靠的是构造，不是靠眼睛看：一次恰好只带必填字段的调用必
// 须被接受，去掉其中任何一个都必须被拒。声明成必填、实际却不需要的字段，过不了
// 前半段；实际需要、却没声明的字段，过不了后半段。
