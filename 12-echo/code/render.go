...
type renderer struct {
	...
	// invalid is tool calls this session rejected at the boundary (stage 11).
	// It sits with the session totals rather than the per-call state because the
	// rate is what means something: a session where a tenth of the calls never
	// made it to a shell is a max_tokens set too low or a schema the model
	// cannot satisfy, and both are invisible one call at a time.
	invalid int

	// cacheHits and cacheBytes are stage 12's result cache, and they are not
	// the same thing as the CacheRead field on Usage a few lines up. That one
	// is the provider's prompt cache and it is a discount on the bill; these
	// two are commands this process did not run. Two features called cache,
	// one panel, and the only defence against reading one number as the other
	// is that they are never printed on the same line.
	cacheHits    int
	cacheMisses  int
	cacheStale   int
	cacheRefused int
	cacheBytes   int
	cacheSaved   int64 // ms of command time that a hit did not spend
	...
	ttft      int64
	idleMax   int64  // stage 10: widest gap in the current stream
	...
	// Retry accounting (stage 09).
	//
	// Two counters, not one, and the split is the correction the first live run
	// forced.
	//
	// retries is the session total and is never reset: it is what a human wants
	// at the end, because "this took four minutes" and "this took four minutes
	// and eleven retries" are different sessions.
	//
	// billedFailures resets on every usage frame and counts only the attempts
	// that reached the model and were therefore charged for — a stream that
	// opened and died. A 503 costs nothing, and a panel that charges for one is
	// inventing money.
	//
	// rebilled stays separate from session for the same class of reason: one is
	// what the provider told us and the other is what we worked out. Merging
	// them would make the measured number and the inferred one
	// indistinguishable in a report, which is how an estimate becomes a fact.
	retries        int
	billedFailures int
	rebilled       Usage
...
// idleNote formats stage 10's instrument, or nothing when there is none.
//
// Nothing, not "idle max 0ms": a stream with one frame has no gap to report,
// and a zero would read as "it never paused" rather than "there was nothing to
// measure". Same rule as the dash on an unknown price.
...
func (r *renderer) OnEvent(e Event) {
	...
	case KindTurnStart:
		r.ttft, r.idleMax, r.sawOutput = 0, 0, false
	...
	case KindToolCallInvalid:
		// Counted as well as printed, and the count is the useful half. One
		// rejected call is noise; a session where a tenth of the calls are
		// `cut` is a max_tokens that is too low, and nothing else on this panel
		// would ever tell you that.
		r.invalid++
		msg := "[malformed call: " + e.Fault + "]"
		if e.Text != "" {
			msg += " " + oneLineDim(e.Text, 68)
		}
		r.p("  %s\n", r.c(cWarn, msg))
	...
	case KindResultCache:
		// Only a hit is printed, and only a hit is counted here. The other
		// three verdicts are the normal weather of a session — a cold cache is
		// all misses and a session doing real work is mostly refusals — and a
		// panel that narrated them would bury the command output under its own
		// bookkeeping. They are all in the trace, which is where you go when
		// you want to know why the hit rate is what it is.
		//
		// A hit prints because it is the one line on this panel that describes
		// something that did NOT happen, and a reader who cannot see it will
		// wonder why a command produced no output and took no time.
		switch cacheVerdict(e.Verdict) {
		case cacheHit:
			r.cacheHits++
			r.cacheBytes += e.Bytes
			r.cacheSaved += e.Millis
			r.p("  %s\n", r.c(cDim, fmt.Sprintf("[cached · %s, not run]", humanBytes(e.Bytes))))
		case cacheMiss:
			r.cacheMisses++
		case cacheStale:
			r.cacheStale++
	...
	case KindUsage:
		if e.Usage != nil {
			...
			// Stage 09: infer what the failed attempts cost.
			//
			// A retried call is billed once per attempt, and the failed
			// attempts cannot say so themselves — usage arrives at the *end* of
			// a stream, and those streams did not end. So the estimate is made
			// here, at the one moment a real number for this prompt exists: the
			// attempt that finally worked.
			//
			// Each failed attempt is priced with the same full/write/read split
			// as the one that succeeded, which makes this a **lower bound** and
			// not a guess in the middle. A cold call pays the cache *write* on
			// its first attempt and the cheaper *read* on the retry, so copying
			// the successful attempt's split under-charges the first one. The
			// summary prints "≥" for that reason.
			//
			// A lower bound is the honest shape for this number anyway: it can
			// be defended in an argument about an invoice, and the alternative
			// on offer from every other agent is silence.
	...
	case KindProvider:
		// The renderer re-prices from this event rather than from startup
		// configuration, and that is the whole reason the prices travel on it.
		//
		// Two payoffs. A session that falls back stops billing the second
		// provider's tokens at the first provider's rates — a cost report that
		// is confidently wrong, which is worse than one that admits it does not
		// know. And a trace replayed on a machine with no providers.json now
		// shows real money, because the rates are in the file.
		if e.Provider != nil {
			...
			if e.Triage == "" {
				break // session start: the banner already named the provider
			}
			r.closeBlock()
	...
	case KindCallError:
		// Printed as a warning, not as an error, when a decision follows it.
		// Most call_errors are survived, and colouring a survivable failure red
		// is how people learn to ignore red.
		r.closeBlock()
	...
	case KindRetry:
		// The retry counter lives here because the renderer is the thing that
		// gets to say what a turn cost, and a retried turn cost more than the
		// panel would otherwise show. See rebilled.
		r.retries++
...
func (r *renderer) renderPanel(e Event) {
	...
	r.p("  %s out %s %s\n",
		...
		r.c(cDim, fmt.Sprintf("TTFT %dms · total %dms%s%s", r.ttft, e.Millis, speed, idleNote(r.idleMax))))
...
func (r *renderer) SessionSummary(finalPrompt int) {
	r.p("\n%s\n", r.c(cDim, "  ── session ──────────────────────"))
	// The malformed count is printed only when it is non-zero. A permanent
	// "0 malformed" on every summary trains people to stop reading the line,
	// which is the opposite of what a number that matters wants.
	if r.invalid > 0 {
		r.p("  %d calls · %d commands · %s\n", r.calls, r.commands,
			r.c(cWarn, fmt.Sprintf("%d malformed", r.invalid)))
	} else {
	...
	// Only printed when they happened. Lines reading "retries: 0" on every clean
	// session teach people to stop reading the block.
	if r.retries > 0 {
		word := "retries"
		if r.retries == 1 {
			word = "retry"
	...
	// The result cache reports even when it did nothing, which is the opposite
	// of the rule the two blocks above follow, and the exception is the point
	// of stage 12. "0 retries" is worth hiding because a session with no
	// retries is a session where nothing went wrong. "0 hits" is worth
	// printing because a cache with no hits looks *exactly* like a cache that
	// is working — no errors, no wrong answers, nothing in any log — and the
	// only thing that distinguishes them is this line. It appears only when the
	// cache is switched on at all.
