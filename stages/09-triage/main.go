// Stage 09 — Triage: the first stage of phase 2, where things start failing.
//
// One idea, and it lives in triage.go: **an error is a decision, not a string.**
// A failed model call gets classified into retry / fall back / stop, and the
// classifier is grounded in docs/wire-notes.md §D11, where the two obvious
// rules — "401 means the key is bad" and "5xx is transient" — are both wrong.
//
// The diff against stage 07 is one new file and a changed error type. What used
// to be
//
//	res, err := a.call(turn, msgs)
//	if err != nil { a.bus.Error("%v", err); return msgs }
//
// is now callWithRetry, and call() itself moved into triage.go as modelCall so
// that stage 05's summariser — which is a real model call and the one every
// agent forgets to instrument — goes through the same decisions instead of its
// own worse copy.
//
// Note the branch point: phase 2 continues from stage 07, not stage 08. Stage
// 08 is the repo's one dependency and is advertised as optional; carrying it
// down the trunk would quietly make it mandatory.
//
// What phase 1 built is unchanged. A subagent is still a function call whose
// return value is a paragraph — it just shares a ladder with its parent now,
// because "this endpoint is refusing calls" is a fact about the endpoint.
package main
...
func main() {
	...
	//
	// It was assembled inline here, once, until the interactive shell. It is a
