...
`--sandbox` 是一句关于整场会话的断言，而一场会话是会派活出去的。以前
第一次 `task` 调用就是它的终点：`newChild` 造子 Agent 的办法，是一个
字段一个字段地点名它继承什么，而沙箱不在这份名单上。子 Agent 拿到的
`sb` 是 nil，`runCommand` 走了另一条分支，派出去的活是在一个真的 bash
里跑的——没有策略，没有 `sandbox_exec` 事件，会话结束时打出的那份统计
里也什么都没有。父 Agent 自己的命令照样受检查，而这正是它看不出来的
原因：凡是读者会想到去看的地方，看上去都是对的。

少掉的那一行只是一个字段，而没人注意到它的原因，比这个字段值钱。子
Agent 是一个字段一个字段拼出来的，而不是写 `child := *a`，这不是口味
问题：`agent` 里有一把 `sync.Mutex`，而 `go vet` 拒绝复制含互斥锁的结
构体，拒得没错。写全了的那种形式是安全的，也是诚实的——它的每一行，
都是在决定子 Agent 到底是什么。而它同时还是——并且是永久地还是——一份
得有人记着往里添的名单。

> **在 `newChild` 写完之后才加到 `agent` 上的每一个字段，都是子 Agent
> 悄无声息拿不到的那个字段。**

同一个函数后来又犯了一次：阶段 10 往 `agent` 上放了三个截止时间，子
Agent 拿不到，而发现这件事要等到阶段 11。什么都没有失败，也没有任何
东西发出警告，因为两种默认行为都会朝某一个方向出错——整体复制会把不
该共享的一起继承过去，逐个写出来则会漏掉必须继承的。真正能接住它的，
是一个把命令穿过子 Agent 跑一遍、再看回来的是什么的测试；
`bypass_test.go` 里那个之所以调 `runCommand`，而不是拿 `child.sb` 和
`parent.sb` 比，就是这个道理。一次重构完全可以照样把指针复制过去，然
后不再用它。

共享沙箱接着会引出第二个问题，而这个问题更有意思。沙箱把它看见的东
西报到一条事件总线上——而一旦一个沙箱要服务整棵树，报到*哪一条*总线
上？在构造时设一个字段是最顺手的答案，也是错的那个：它拿到的会是根
Agent 的总线，于是子 Agent 产生的每一次 exec、每一次 open、每一次拒
绝，送达时都会被标上深度 0、Agent 名字为空。这份 trace 会是完整的、
顺序也正确的，只是名字标错了，还一点不像标错——这比少一行更糟：少一
行会留下一个豁口，而豁口是会被人看见的。

所以总线是 `run` 的一个参数，不是 `sandbox` 上的一个字段。一条命令是
哪个 Agent 跑的，是这次调用的属性；策略和审计记录，则是整棵树的属
性。这和 `render.go` 划的是同一条界线——它从事件上读深度，而不是把
"我们此刻在哪个 agent 里面"当成渲染器状态来跟踪——理由也一样：阶段 07
让这棵树并发起来了，于是一件事*什么时候*发生，不再足以说明它是*谁*干
的。反正每条命令都会新建一个解释器，所以它拿到的那些 handler 把答案闭
包进去，是白拿的。
