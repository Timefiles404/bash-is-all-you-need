...
func (s *session) godLine(e Event, w int) []string {
	...
	case KindResultCache:
		// The god view shows all four verdicts where the terminal panel shows
		// only hits. The two have different jobs: the panel is a live
		// instrument that must not drown the output it is annotating, and this
		// is the forensic view, where "why did this session never hit" is a
		// question you came here to answer and the refusal reasons are the
		// answer.
		if e.Verdict == string(cacheHit) {
			return line(sBold, "cache_hit", fmt.Sprintf("%s served · %dms not spent · %s",
				humanBytes(e.Bytes), e.Millis, dim(oneLine(e.Command, w-52))))
		}
		why := e.Text
		if why == "" {
			why = oneLine(e.Command, w-40)
		}
		return line(sDim, "cache_"+e.Verdict, dim(oneLine(why, w-36)))
